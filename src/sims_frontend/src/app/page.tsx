...
export default function DashboardContent() {
  ...
  const [selectedEquipmentList, setSelectedEquipmentList] = useState<
    Equipment[]
  >([]); // Stores full selected equipment objects
    ...
    const fetchSchool = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_BASE}/api/school/${us?.schoolId}`,
          {
            headers: {
  ...
  const filteredEquipment = useMemo(() => {
    ...
    let currentFilteredList = school?.equipment?.filter(
      (eq) =>
        !selectionMode ||
        !selectedEquipmentList.some((selectedEq) => selectedEq.id === eq.id),
    );
    ...
    if (filterCondition) {
      currentFilteredList = currentFilteredList?.filter(
        (equipment) => equipment.status === filterCondition,
      );
    }

    if (filterType) {
      currentFilteredList = currentFilteredList?.filter(
        (equipment) => equipment.type === filterType,
      );
    }
  ...
  const handleCheckboxChange = useCallback(
    (equipment: Equipment, isSelected: boolean) => {
      if (isSelected) {
        ...
        setSelectedEquipmentList((prevSelected) =>
          prevSelected.filter((eq) => eq.id !== equipment.id),
        );
      }
    },
    [],
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {(selectionMode || selectedEquipmentList.length > 0) && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-8">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
            Selected Equipment ({selectedEquipmentList.length})
          </h2>
          {selectedEquipmentList.length > 0 ? (
                ...
                <tbody>
                  {selectedEquipmentList.map((equipment) => (
                    <tr
                      ...
                      className="bg-blue-50 dark:bg-blue-950 border-b dark:border-gray-700"
                    >
                      <td className="w-4 p-4">
                        <input
                          ...
                          onChange={() =>
                            handleCheckboxChange(equipment, false)
                          } // Deselect
                        />
                      </td>
                      <th
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          <div
                            className={`h-2.5 w-2.5 rounded-full ${getConditionColor(equipment.status)} me-2`}
                          ></div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              No equipment currently selected.
            </p>
          )}
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-8">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
          Available Equipment (Browse & Filter)
        </h2>
                  ...
                  {selectionMode && (
                    <div className="flex items-center">
                      <input
                          ...
                          if (e.target.checked && availableFilteredEquipment) {
                            setSelectedEquipmentList((prevSelected) => [
                              ...
                              ...availableFilteredEquipment.filter(
                                (eq) =>
                                  !prevSelected.some((pEq) => pEq.id === eq.id),
                              ),
                            ]);
                          } else {
                            setSelectedEquipmentList((prevSelected) =>
                              prevSelected.filter(
                                (eq) =>
                                  !availableFilteredEquipment?.some(
                                    (aEq) => aEq.id === eq.id,
                                  ),
                              ),
                            );
                        ...
                        checked={
                          availableFilteredEquipment &&
                          availableFilteredEquipment.length > 0 &&
                          availableFilteredEquipment.every((eq) =>
                            selectedEquipmentList.some(
                              (sEq) => sEq.id === eq.id,
                            ),
                          )
                        }
                    ...
                    {selectionMode && (
                      <div className="flex items-center">
                        <input
                          ...
                          type="checkbox"
                          className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                          checked={selectedEquipmentList.some(
                        ...
                        />
                        <label
                          htmlFor={`checkbox-table-search-${equipment.id}`}
                          className="sr-only"
                        >
                          checkbox
                        </label>
