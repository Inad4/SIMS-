...
export default function SignUpPage() {
  ...
  const [schoolMenuSelectedSchool, setSchoolMenuSelectedSchool] =
    useState<School | null>(null);
  const [schoolMenuShowDropdown, setSchoolMenuShowDropdown] =
    ...
    const fetchSchools = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_BASE}/api/School`,
        );
        if (!res.ok) {
  ...
  const handleSignUp = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_AUTH_BASE}/signup`,
        {
          method: "POST",
          headers: {
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            >
              ...
              <div className="relative" ref={comboboxRef}>
                ...
                {schoolMenuShowDropdown &&
                  filteredSchools.length === 0 &&
                  schoolMenuSearchTerm && (
